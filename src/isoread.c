...
static int iso_type_check(SceUID fd)
{
    ...
    *((u32*)&g_hdr) = 0;
    ...
    ret = sceIoRead(fd, &g_hdr, sizeof(g_hdr));
    ...
    if (magic == CSO_MAGIC || magic == ZSO_MAGIC || magic == DAX_MAGIC || magic == JSO_MAGIC) { // CISO or ZISO or JISO or DAX
        ...
        if (magic == DAX_MAGIC){
            ...
            uncompressed_size = g_hdr.DAX.uncompressed_size;
            block_header = 2; // skip over the zlib header (2 bytes)
            align = 0; // no alignment for DAX
            com_size = DAX_COMP_BUF;
            ciso_decompressor = (g_hdr.DAX.version >= 1)? &decompress_dax1 : &decompress_dax0;
        }
        else if (magic == JSO_MAGIC){
            header_size = sizeof(JISOHeader);
            block_size = g_hdr.JISO.block_size;
            uncompressed_size = g_hdr.JISO.uncompressed_size;
            block_header = 4*g_hdr.JISO.block_headers; // if set to 1, each block has a 4 byte header, 0 otherwise
            align = 0; // no alignment for JISO
            com_size = g_hdr.JISO.block_size;
            ciso_decompressor = (g_hdr.JISO.method)? &decompress_dax1 : &decompress_jiso; //  zlib or lzo, depends on method
        }
        else{ // CSO/ZSO/v2
            header_size = sizeof(CISOHeader);
            block_size = g_hdr.CISO.block_size;
            uncompressed_size = g_hdr.CISO.total_bytes;
            block_header = 0; // CSO/ZSO uses raw blocks
            align = g_hdr.CISO.align;
            com_size = block_size + (1 << g_hdr.CISO.align);
            if (g_hdr.CISO.ver == 2) ciso_decompressor = &decompress_cso2; // CSOv2 uses both zlib and lz4
